//! Leitmotif's frontend: input in, shapes and sound out.
//!
//! Everything that decides what happens lives in the library's
//! `leitmotif::sim`. This file only translates between the shell's frame and
//! the sim's world; the sim's `Cue`s become sound over in [`audio`].
//!
...
//! player through an unbounded world; the HUD sits still in the shell's
//! letterboxed frame.
...
use game_prototypes::leitmotif::sim::{
...
use game_prototypes::leitmotif::song::{
    BEATS_PER_BAR, Instrument, Layer, Motif, SONG, STEPS_PER_BAR,
};

use crate::ui::{self, DIM, Frame, HIGHLIGHT, MqVec2, OVERLAY, SHADE, pulse, with_alpha};

/// The shell's frame, under the names the drawing code below already uses.
const VIEW_W: f32 = ui::FRAME_W;
const VIEW_H: f32 = ui::FRAME_H;
...
/// A run of Leitmotif, with the sound bank it needs.
pub struct Game {
    sim: Sim,
    audio: Audio,
    camera: Camera,
...
impl Game {
    /// Synthesise and decode the whole sound bank, then start a run.
    ///
    /// Async because on the web every buffer goes to the browser to decode
    /// and macroquad waits frames for the result, so this is a visible
    /// pause. The shell loads a prototype once and keeps it.
    pub async fn load() -> Self {
        let sim = Sim::new(fresh_seed());
        let camera = Camera::at(sim.player().body.pos);
        Self {
...
impl crate::games::Game for Game {
    fn update(&mut self, dt: f32) {
        ...
        self.audio.update(self.sim.cues(), &input, toggle_mute);
        self.hud.update(dt, self.sim.cues());
        self.camera.follow(&self.sim, dt);
    }

    fn draw(&self, frame: &Frame) {
        let view = View::new(*frame, self.camera.pos);
        draw(&self.sim, &view, &self.audio, &self.hud, self.clock);
    }
}

/// Wall-clock seed. Swap in `quad-url`/// Wall-clock seed. Swap in `quad-url` here if you want seeds that survive in
...
/// The world scrolling under the shell's fixed frame.
///
/// Everything here is a thin wrapper over [`Frame`]: [`View::world`] adds the
/// camera, [`View::hud`] does not, and the primitives translate this file's
/// vectors into the shell's before handing the drawing over.
struct View {
    frame: Frame,
...
impl View {
    const fn new(frame: Frame, camera: Vec2) -> Self {
        Self { frame, camera }
    }
    ...
    fn hud(&self, at: Vec2) -> Vec2 {
        from_mq(self.frame.at(to_mq(at)))
    }
    ...
    // Primitives take pixel positions and frame-unit sizes, as the shell's
    // frame does; these only convert the vector type.

    fn circle(&self, p: Vec2, radius: f32, color: Color) {
        self.frame.circle(to_mq(p), radius, color);
    }

    fn ring(&self, p: Vec2, radius: f32, thickness: f32, color: Color) {
        self.frame.ring(to_mq(p), radius, thickness, color);
    }
    ...
    fn arc(
        ...
        self.frame
            .arc(to_mq(p), radius, thickness, from_deg, span_deg, color);
    }

    fn rect(&self, p: Vec2, size: Vec2, color: Color) {
        self.frame.rect(to_mq(p), to_mq(size), color);
    }

    fn rect_lines(&self, p: Vec2, size: Vec2, thickness: f32, color: Color) {
        self.frame
            .rect_lines(to_mq(p), to_mq(size), thickness, color);
    }

    fn line(&self, a: Vec2, b: Vec2, thickness: f32, color: Color) {
        self.frame.line(to_mq(a), to_mq(b), thickness, color);
    }

    fn triangle(a: Vec2, b: Vec2, c: Vec2, color: Color) {
        Frame::triangle(to_mq(a), to_mq(b), to_mq(c), color);
    }

    /// A keyboard key of this size, centred on `p`.
    fn key_cap(&self, ch: Option<char>, p: Vec2, size: Vec2, color: Color) {
        self.frame.key_cap(ch, to_mq(p), to_mq(size), color);
    }

    /// A key cap with a solid triangle on it, pointing along `dir`.
    fn arrow_cap(&self, dir: Vec2, p: Vec2, size: f32, color: Color) {
        self.frame.arrow_cap(to_mq(dir), to_mq(p), size, color);
    }
}

const fn to_mq(v: Vec2) -> MqVec2 {
    MqVec2::new(v.x, v.y)
}

const fn from_mq(v: MqVec2) -> Vec2 {
    Vec2::new(v.x, v.y)
}
...
/// Concentric rings, breathing. The same mark stands for home in the world,
/// in the HUD, and in the menu's emblem.
fn home_mark(frame: &Frame, p: MqVec2, radius: f32, clock: f32, color: Color) {
    let breath = (clock * 2.0).sin().mul_add(0.06, 1.0);
    frame.ring(p, radius * breath, radius * 0.08, with_alpha(color, 0.9));
    frame.ring(
        p,
        radius * 0.62 * breath,
        radius * 0.08,
    ...
    frame.circle(p, radius * 0.22, color);
}

fn home_glyph(view: &View, p: Vec2, radius: f32, clock: f32, color: Color) {
    home_mark(&view.frame, to_mq(p), radius, clock, color);
}
...
fn draw_title(view: &View, clock: f32) {
    ...
    fermata_glyph(
        view,
        space - Vec2::new(0.0, 40.0 * view.scale()),
        32.0,
        soft,
    );
}
...
/// The mark the menu shows for this prototype: you, the lit way, and home,
/// over a bar of the transport strip that runs everything.
pub fn emblem(frame: &Frame, centre: MqVec2, width: f32, clock: f32) {
    /// Dots along the path between you and home.
    const STEPS: usize = 11;

    let s = frame.scale();
    let reach = width * 0.5 * s;
    let you = centre - MqVec2::new(reach, width * 0.06 * s);
    let home = centre + MqVec2::new(reach, -width * 0.06 * s);

    for i in 1..STEPS {
        let t = i as f32 / STEPS as f32;
        // One glow travelling the path, so the emblem reads as a journey.
        let phase = t
            .mul_add(STEPS as f32, -(clock * 3.0))
            .rem_euclid(STEPS as f32);
        let glow = (1.0 - phase / 3.0).max(0.0);
        frame.circle(
            you.lerp(home, t),
            width * 0.012,
            with_alpha(HOME_GLOW, glow.mul_add(0.6, 0.2)),
        );
    }
    frame.circle(you, width * 0.05, with_alpha(PLAYER, 0.12));
    frame.circle(you, width * 0.028, PLAYER);
    home_mark(frame, home, width * 0.085, clock, HOME_GLOW);

    // A bar of the transport strip below: the thing that runs everything.
    let accent = palette_for(Motif::Wander).accent;
    let step_w = width / STEPS_PER_BAR as f32;
    let live = (clock * 6.0).rem_euclid(STEPS_PER_BAR as f32).floor();
    for step in 0..STEPS_PER_BAR {
        let on_beat = step % (STEPS_PER_BAR / BEATS_PER_BAR) == 0;
        let height = if on_beat { 7.0 } else { 4.5 };
        let colour = if (step as f32 - live).abs() < 0.5 {
            accent
        } else {
            with_alpha(accent, if on_beat { 0.35 } else { 0.18 })
        };
        let x = (step as f32).mul_add(step_w, -width * 0.5);
        frame.rect(
            centre + MqVec2::new(x * s, width * 0.16 * s),
            MqVec2::new(step_w - 1.5, height),
            colour,
        );
    }
}
